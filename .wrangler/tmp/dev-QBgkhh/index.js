...
function deriveRiskTier2(score) {
  if (score >= 85) return "critical";
  if (score >= 65) return "high";
  if (score >= 35) return "medium";
  return "low";
...
function normalizeEmail(value) {
  return normalizeText4(value).toLowerCase();
}
__name(normalizeEmail, "normalizeEmail");
function normalizeAnnotationPayload(payload) {
  return {
    annotation: normalizeText4(payload.annotation || payload.note)
  };
}
...
function normalizeIdentityPayload(payload, fallbackSourcePage, request) {
  const identity = {
    ...
    operator_handle: normalizeText4(payload.operator_handle || payload.name || payload.operatorHandle),
    ...
    transmission: normalizeText4(payload.transmission || payload.message || payload.notes),
    source_page: normalizeText4(payload.source_page || payload.source || fallbackSourcePage) || void 0,
    ...
    status: normalizeText4(payload.status) || "new"
  };
...
function normalizeEngagementPayload(payload) {
  return {
    ...
    operatorHandle: normalizeText4(payload.operator_handle || payload.name || payload.operatorHandle || payload.contactName),
    organization: normalizeNullable2(payload.organization || payload.org || payload.company),
    contactEmail: normalizeEmail(payload.contact_email || payload.email || payload.contactEmail),
    transmission: normalizeText4(payload.transmission || payload.message || payload.notes),
    source: normalizeText4(payload.source || payload.source_page) || "landing",
...
function normalizeIntentPayload(payload) {
  return {
    intent: normalizeText4(payload.intent),
    source: normalizeText4(payload.source) || "public-landing",
    agent: normalizeText4(payload.agent) || "route-advisory"
  };
}
...
async function createIntegration(env, payload) {
  const endpoint = normalizeText4(payload.endpoint);
  const integration = {
    id: buildId("int"),
    name: normalizeText4(payload.name) || "Unnamed Integration",
    type: normalizeText4(payload.type) || "api",
    endpoint,
    permissions: Array.isArray(payload.permissions) ? payload.permissions.map((entry) => normalizeText4(entry)).filter(Boolean) : [],
    createdAt: (/* @__PURE__ */ new Date()).toISOString()
  };
...
async function resolveIntegrationContext(env, value, requiredPermission) {
  const integrationId = normalizeText4(value);
  if (!integrationId) {
    return null;
  }
...
async function evaluateSafetyCheck(env, payload) {
  ...
  const intent = normalizeText4(payload.intent);
  const agent = normalizeText4(payload.agent);
  const pipelineName = normalizeText4(payload.pipeline);
  const scenarioName = normalizeText4(payload.scenario);
  const reasons = [];
  if (agent) {
    const rule = governance.agentSafetyRules?.[agent];
    if (rule?.enabled === false) {
...
async function enforceGovernance(env, surface, payload, options = {}) {
  ...
  const agent = normalizeText4(options.agent || payload.agent);
  if (integrationId) {
...
async function runCertification(env, payload) {
  ...
  const tests = Array.isArray(payload.tests) && payload.tests.length ? payload.tests.map((entry) => normalizeText4(entry)).filter(Boolean) : ["route-smoke", "payload-shape", "safety-compat"];
  const results = [];
...
async function applyRelease(env, payload) {
  const version = normalizeText4(payload.version);
  if (!version) {
    throw new Error("version is required");
  }
  const changes = Array.isArray(payload.changes) ? payload.changes.map((entry) => normalizeText4(entry)).filter(Boolean) : [];
  const migrations = Array.isArray(payload.migrations) ? payload.migrations.map((entry) => normalizeText4(entry)).filter(Boolean) : [];
  const release = {
    id: `rel-${Date.now()}`,
    version,
    changes,
...
async function computeGlobalRoutePlan(env, payload) {
  ...
  const normalizedIntent = normalizeText4(payload.intent);
  const mode = normalizeText4(payload.mode) || "auto";
...
async function saveDivisionMemoryEntry(env, payload) {
  const key = normalizeText4(payload.key || payload.id);
  if (!key) {
  ...
  const record = {
    id: key,
    key,
    type: normalizeText4(payload.type) || "operator-note",
    value: payload.value ?? payload.note ?? payload.metadata ?? null,
    summary: normalizeText4(payload.summary) || normalizeText4(typeof payload.value === "string" ? payload.value : key),
    updatedAt: (/* @__PURE__ */ new Date()).toISOString()
  };
...
async function runPipeline(env, payload, source = "pipeline-engine") {
  const moduleId = normalizeText4(payload.moduleId);
  ...
  const governance = await enforceGovernance(
    ...
    {
      ...payload,
      intent: normalizeText4(payload.intent) || `Pipeline for ${moduleId}`,
      integrationId: getIntegrationIdFromPayload(payload)
    },
    {
      requestedChainLength: steps.length,
      requiredPermission: "pipeline",
      chain: steps.map((step) => normalizeText4(step.agent)).filter(Boolean)
    }
  );
  await assertCertifiedAgents(
    env,
    steps.map((step) => normalizeText4(step.agent)).filter(Boolean),
    "pipeline",
    governance
  );
  ...
  for (const step of steps) {
    ...
    const flow = await executeAgentFlow(
      env,
      {
        intent: normalizeText4(step.intent) || moduleRecord?.summary || `Pipeline for ${moduleId}`,
        source,
        agent: step.agent
      },
      {
...
async function runSandbox(env, payload) {
  const agent = normalizeText4(payload.agent);
  const input = normalizeText4(payload.input);
...
async function createOperatorIntent(env, payload) {
  const goal = normalizeText4(payload.goal);
  if (!goal) {
  ...
  const routePlan = await computeGlobalRoutePlan(env, {
    ...
    mode: "manual",
    context: {
      priority: normalizeText4(payload.priority) || "medium",
...
async function runPublicScenario(env, payload) {
  const name = normalizeText4(payload.name);
  const steps = Array.isArray(payload.steps) ? payload.steps : [];
  ...
  const governance = await enforceGovernance(
    ...
    },
    {
      ...
      chain: steps.map((step) => lightweightSemanticScore(normalizeText4(step.intent)).recommendedAgent)
    }
  );
  const plannedAgents = [];
  for (const step of steps) {
    const previewPlan = await computeGlobalRoutePlan(env, {
      intent: normalizeText4(step.intent),
      mode: "auto",
...
async function runScenario(env, payload) {
  ...
  for (const step of steps) {
    ...
    const intent = normalizeText4(step.intent) || normalizeText4(previousOutput?.envelope?.content) || name;
    const flow = await executeAgentFlow(
      env,
...
async function executeOperatorCommand(env, commandText) {
  const command = normalizeText4(commandText);
  if (!command) {
...
async function handleMarketplaceSearch(request, env, url, logSearches = true) {
  const term = normalizeText4(url.searchParams.get("q"));
...
async function handleApi(request, env, url) {
  ...
  if (method === "POST" && pathname === "/api/os/version") {
    try {
      ...
      const nextVersion = {
        current: normalizeText4(body.current) || current.current,
        history: Array.isArray(body.history) && body.history.length ? body.history : [.../* @__PURE__ */ new Set([...current.history, normalizeText4(body.current) || current.current])],
  ...
  if (method === "GET" && pathname === "/api/events") {
    try {
      const type = normalizeText4(url.searchParams.get("type"));
  ...
  if (method === "GET" && pathname === "/api/operator/audit") {
    try {
      const action = normalizeText4(url.searchParams.get("action"));
  ...
  if (method === "GET" && pathname === "/api/engagements") {
    return json({ engagements: engagements2, packages: packages2 });
  }
  ...
  if (method === "GET" && pathname === "/api/marketplace/service-modules") {
    return json({
      modules: [...serviceMarketplaceModules2, auditLiteMarketplaceModule2, promptInjectionMarketplaceModule2]
    });
  }
  if (method === "GET" && pathname === "/api/deliverables") {
    return json({ deliverables: deliverables2 });
  }
...
// .wrangler/tmp/bundle-isDbxG/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-isDbxG/middleware-loader.entry.ts
