...
var require_contracts = __commonJS({
  "data/contracts.js"(exports, module) {
    init_modules_watch_stub();
    ...
    module.exports = {
      ...
      IDENTITY_STATUSES,
      componentMapping,
      deploymentReference: deploymentReference2
    };
  }
});
...
var require_validate = __commonJS({
  "data/validate.js"(exports, module) {
    init_modules_watch_stub();
    var {
      MODULE_STATUSES,
      MODULE_CTA_LABELS,
    ...
    function validateIdentityRecord2(record) {
    ...
    __name(validateIdentityRecord2, "validateIdentityRecord");
...
var require_store = __commonJS({
  "data/store.js"(exports, module) {
    init_modules_watch_stub();
    var { randomUUID } = require_crypto();
    var {
      validateCollection,
      validateModuleRecord,
      validatePackageRecord,
    ...
    var moduleRegistry2 = validateCollection([
      {
    ...
    var packageCatalog = validateCollection([
      {
    ...
    var deliverableCatalog = validateCollection([
      {
        id: "threat-model-map",
        artifact_num: "01",
    ...
    var identities2 = validateCollection([
      {
    ...
    function getModuleRoute2(id) {
      return `/marketplace/modules/${id}`;
    }
    __name(getModuleRoute2, "getModuleRoute");
    function getModuleStaticPath2(id) {
      return `/modules/${id}.html`;
    }
    __name(getModuleStaticPath2, "getModuleStaticPath");
    function toModulePayload(record) {
      ...
      return {
        ...
        metadata: {
          ...
          staticPath: getModuleStaticPath2(record.id),
          ctaLabel: record.cta_label,
          ctaHref: record.cta_href || getModuleRoute2(record.id),
    ...
    function createId2(prefix, collection) {
      const next = collection.length + 1001;
      return `${prefix}-${next}`;
    }
    __name(createId2, "createId");
    function createIdentityId2() {
      return randomUUID();
    }
    __name(createIdentityId2, "createIdentityId");
    var modules2 = moduleRegistry2.map(toModulePayload);
    var packages2 = packageCatalog.map((entry) => ({
      ...entry,
      outcomes: [...entry.outcomes],
      scope: [...entry.scope]
    }));
    var deliverables2 = deliverableCatalog.map(toDeliverablePayload);
    module.exports = {
...
// data/serviceSelector.js
var require_serviceSelector = __commonJS({
  "data/serviceSelector.js"(exports, module) {
    init_modules_watch_stub();
    var serviceCatalog2 = [
      {
    ...
    var serviceMarketplaceModules2 = [
      {
    ...
    function normalizeSelectorAnswers2(payload = {}) {
      const usage = clampCatalogUsage(payload.current_ai_usage);
      const primaryGoal = normalizeText2(payload.primary_goal) || "not_sure";
      const businessType = normalizeText2(payload.business_type);
      const riskLevel = normalizeText2(payload.risk_level) || "unknown";
      const budgetRange = normalizeText2(payload.budget_range) || "not_sure";
      const urgency = normalizeText2(payload.urgency) || "research_only";
      return {
        ...
        source_route: normalizeText2(payload.source_route) || "/services"
      };
    }
    __name(normalizeSelectorAnswers2, "normalizeSelectorAnswers");
    function computeServiceSelectorResult2(answers, selectorId = generateSelectorId()) {
      const usage = answers.current_ai_usage || [];
    ...
    __name(computeServiceSelectorResult2, "computeServiceSelectorResult");
    function getServiceBySlug(slug) {
      return serviceCatalog2.find((service) => service.slug === slug);
    }
    ...
    function recordServiceSelectorSubmission2(answers, result) {
    ...
    __name(recordServiceSelectorSubmission2, "recordServiceSelectorSubmission");
    function attachEngagementToSelector2(details = {}) {
      const selectorId = normalizeText2(details.selector_id || details.selectorId);
      if (!selectorId) {
        return null;
      }
      ...
      const base = existing || {
        ...
        recommended_service: normalizeText2(details.recommended_service || details.moduleInterest || details.service) || "ai_security_audit",
        secondary_service: normalizeNullable2(details.secondary_service),
        urgency_score: Number.isFinite(Number(details.urgency_score)) ? Number(details.urgency_score) : 0,
        revenue_potential: normalizeText2(details.revenue_potential) || "medium",
        priority: normalizeText2(details.priority) || "medium",
        next_route: "",
        status: "service-match-ready"
      };
      ...
      base.recommended_service = normalizeText2(details.recommended_service || details.moduleInterest || details.service) || base.recommended_service;
      base.secondary_service = normalizeNullable2(details.secondary_service) || base.secondary_service || null;
      base.priority = normalizeText2(details.priority) || base.priority || "medium";
      base.revenue_potential = normalizeText2(details.revenue_potential) || base.revenue_potential || "medium";
    ...
    __name(attachEngagementToSelector2, "attachEngagementToSelector");
    function listServiceIntakeQueue2(engagements2 = []) {
      const engagementMap = new Map(
        engagements2.filter((entry) => entry.selectorId || entry.recommendedService).map((entry) => [
      ...
      for (const submission of serviceSelectorSubmissions) {
        const engagement = submission.selector_id ? engagementMap.get(submission.selector_id) : null;
        queue.push({
          selector_id: submission.selector_id,
          engagement_id: submission.engagement_id || engagement?.engagement_id || null,
          recommended_service: submission.recommended_service || engagement?.recommended_service || null,
          secondary_service: submission.secondary_service || engagement?.secondary_service || null,
          urgency_score: submission.urgency_score ?? engagement?.urgency_score ?? 0,
          revenue_potential: submission.revenue_potential || engagement?.revenue_potential || "medium",
          priority: submission.priority || engagement?.priority || "medium",
          status: engagement?.status || submission.status || "service-match-ready",
          created_at: engagement?.created_at || submission.created_at || null
        });
      }
      for (const engagement of engagements2) {
        ...
        queue.push({
          selector_id: engagement.selectorId || null,
          engagement_id: engagement.id,
          recommended_service: engagement.recommendedService || engagement.moduleInterest || null,
          secondary_service: engagement.secondaryService || null,
          urgency_score: engagement.urgencyScore || 0,
          revenue_potential: engagement.revenuePotential || "medium",
          priority: engagement.priority || engagement.urgency || "medium",
    ...
    __name(listServiceIntakeQueue2, "listServiceIntakeQueue");
    module.exports = {
      selectorIdPattern,
...
// data/auditLite.js
var require_auditLite = __commonJS({
  "data/auditLite.js"(exports, module) {
    init_modules_watch_stub();
    var auditLiteMarketplaceModule2 = {
    ...
    function normalizeAuditLiteAnswers2(payload = {}) {
      return {
    ...
    __name(normalizeAuditLiteAnswers2, "normalizeAuditLiteAnswers");
    ...
    function computeAuditLiteResult2(answers, auditId = generateAuditLiteId()) {
    ...
    __name(computeAuditLiteResult2, "computeAuditLiteResult");
    ...
    function recordAuditLiteSubmission2(answers, result) {
    ...
    __name(recordAuditLiteSubmission2, "recordAuditLiteSubmission");
    function attachEngagementToAuditLite2(details = {}) {
    ...
    __name(attachEngagementToAuditLite2, "attachEngagementToAuditLite");
    function listAuditLiteQueue2(engagements2 = []) {
      const queue = [];
      for (const submission of auditLiteSubmissions) {
        const linkedEngagement = engagements2.find((entry) => entry.auditId && entry.auditId === submission.audit_id);
        queue.push({
    ...
    __name(listAuditLiteQueue2, "listAuditLiteQueue");
    module.exports = {
...
// data/promptInjectionScanner.js
var require_promptInjectionScanner = __commonJS({
  "data/promptInjectionScanner.js"(exports, module) {
    init_modules_watch_stub();
    var promptInjectionMarketplaceModule2 = {
    ...
    function sanitizePromptPreview(value) {
      return normalizeText2(value).replace(/\s+/g, " ").replace(/[<>{}]/g, "").slice(0, 120) || null;
    }
    ...
    __name(derivePriority, "derivePriority");
    function normalizePromptInjectionAnswers2(payload = {}) {
      const allowPromptText = Boolean(payload.allow_prompt_text);
      const normalizedPromptText = allowPromptText ? normalizeText2(payload.prompt_text).slice(0, 4e3) : "";
      return {
        ...
        source_route: normalizeText2(payload.source_route) || "/apps/prompt-injection-scanner"
      };
    }
    __name(normalizePromptInjectionAnswers2, "normalizePromptInjectionAnswers");
    ...
    function computePromptInjectionResult2(answers, scanId = generatePromptInjectionScanId()) {
    ...
    __name(computePromptInjectionResult2, "computePromptInjectionResult");
    ...
    function recordPromptInjectionSubmission2(answers, result) {
    ...
    __name(recordPromptInjectionSubmission2, "recordPromptInjectionSubmission");
    function attachEngagementToPromptInjectionScan2(details = {}) {
      const scanId = normalizeText2(details.scan_id || details.scanId);
      if (!scanId) {
        return null;
      }
      ...
      base.priority = normalizeText2(details.priority) || base.priority || "low";
      base.status = normalizeText2(details.status) || "intake-received";
      base.recommended_service = normalizeText2(details.recommended_service || details.recommendedService) || "prompt_injection_review";
      base.secondary_service = normalizeText2(details.secondary_service || details.secondaryService) || base.secondary_service || "ai_security_audit";
      if (Array.isArray(details.top_risks) && details.top_risks.length) {
        base.top_risks = details.top_risks;
      }
      return upsertPromptInjectionSubmission(base);
    }
    __name(attachEngagementToPromptInjectionScan2, "attachEngagementToPromptInjectionScan");
    function listPromptInjectionScanQueue2(engagements2 = []) {
      const queue = [];
      for (const submission of promptInjectionSubmissions) {
        const linkedEngagement = engagements2.find((entry) => entry.scanId && entry.scanId === submission.scan_id);
        queue.push({
    ...
    __name(listPromptInjectionScanQueue2, "listPromptInjectionScanQueue");
    module.exports = {
...
// data/cloudflareMcp.js
var require_cloudflareMcp = __commonJS({
  "data/cloudflareMcp.js"(exports, module) {
    init_modules_watch_stub();
    var CLOUDFLARE_MCP_SERVERS2 = {
      cloudflare: {
        id: "cloudflare",
    ...
    var WRANGLER_BINDINGS_MANIFEST2 = {
    ...
    var PIPELINE_REQUIRED_BINDINGS2 = ["MODULES", "PIPELINES", "ASSETS", "GOVERNANCE"];
    var CURATED_DOCS_INDEX2 = [
    ...
    var CLOUDFLARE_FEDERATION_ACTIONS2 = [
    ...
    var MODULE_CF_ACTION_COMPATIBILITY2 = {
    ...
    var AUTONOMOUS_SIGNAL_TRIGGERS2 = {
      "logs-anomaly": {
        id: "logs-anomaly",
        label: "Logs anomaly",
    ...
    var ANOMALY_LOG_PATTERNS2 = [/error/i, /fail/i, /timeout/i, /degraded/i, /warning/i, /exception/i, /spike/i];
    var METRICS_SPIKE_KEYWORDS2 = [/spike/i, /elevated/i, /high/i, /surge/i];
    var CLOUDFLARE_EVENT_HOOKS2 = {
      onBuildComplete: {
        id: "onBuildComplete",
    ...
    var CLOUDFLARE_INSIGHTS_RECOMMENDATIONS2 = [
      {
...
// .wrangler/tmp/bundle-UfXnwR/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-UfXnwR/middleware-insertion-facade.js
init_modules_watch_stub();
...
var import_serviceSelector = __toESM(require_serviceSelector());
var import_auditLite = __toESM(require_auditLite());
var import_promptInjectionScanner = __toESM(require_promptInjectionScanner());

// worker/cloudflare-integration.js
init_modules_watch_stub();
var import_cloudflareMcp = __toESM(require_cloudflareMcp());
...
async function runCloudflareBuild(options = {}) {
  ...
  const logs = listBuilds.result ? listBuilds.result.split("\n").filter(Boolean).slice(0, 50) : STATIC_BUILD_MANIFEST.logs;
  return {
...
function getDocsQuickActions(category = null) {
  const filtered = category ? DOCS_QUICK_ACTIONS.filter((action) => action.category === normalizeDocsTopic(category) || action.topic === normalizeDocsTopic(category)) : DOCS_QUICK_ACTIONS;
...
function simulateCloudflareEventHooks(autonomousSignals = {}, inputs = {}) {
  ...
  const buildComplete = !signals.buildFailure?.triggered && (inputs.build?.health === "online" || inputs.build?.source === STATIC_BUILD_MANIFEST.source);
...
function deriveCrossDivisionHealth(score = 100, syncStatus = "aligned") {
...
function deriveOrchestrationHealth(score = 100, plan = []) {
...
function deriveExecutionHealth(score = 100, executionPlan = []) {
  const highPriority = executionPlan.filter((step) => step.priority === "high").length;
  if (score >= 80 && highPriority === 0) {
    return "healthy";
  }
...
function deriveAdaptiveHealth(score, signalsMissing = false) {
...
var {
  ...
  getModuleRoute,
  getModuleStaticPath
} = import_store.default;
var { deploymentReference } = import_contracts.default;
var { validateIdentityRecord } = import_validate.default;
var {
  serviceCatalog,
...
} = import_serviceSelector.default;
var {
...
} = import_promptInjectionScanner.default;
var DEFAULT_HEADERS = { ...deploymentReference.headers };
var STORE_CONFIG = {
...
var worker_default = {
  async fetch(request, env) {
    try {
      ...
      console.log("[diag] fetch-start", request.method, url.pathname);
      ...
      console.error("[diag] fetch-failed", error?.stack || String(error));
      return json(
        {
          diagnostic: "worker-fetch-failed",
...
function resolveStaticPath(requestPath) {
  ...
  const moduleRoute = requestPath.match(/^\/marketplace\/modules\/([a-z0-9-]+)$/);
  if (moduleRoute) {
...
function normalizeIdentityPayload(payload, fallbackSourcePage, request) {
  const identity = {
    ...
    source_page: normalizeText(payload.source_page || payload.source || fallbackSourcePage) || void 0,
...
async function createIntegration(env, payload) {
  ...
  const integration = {
    ...
    endpoint,
    permissions: Array.isArray(payload.permissions) ? payload.permissions.map((entry) => normalizeText(entry)).filter(Boolean) : [],
    createdAt: (/* @__PURE__ */ new Date()).toISOString()
  };
...
async function runCertification(env, payload) {
  ...
  const tests = Array.isArray(payload.tests) && payload.tests.length ? payload.tests.map((entry) => normalizeText(entry)).filter(Boolean) : ["route-smoke", "payload-shape", "safety-compat"];
  const results = [];
...
async function handleApi(request, env, url) {
  ...
  if (method === "POST" && pathname === "/api/os/version") {
    try {
      ...
      const nextVersion = {
        ...
        history: Array.isArray(body.history) && body.history.length ? body.history : [.../* @__PURE__ */ new Set([...current.history, normalizeText(body.current) || current.current])],
  ...
  if (method === "GET" && pathname === "/api/os/cloudflare/logs") {
    try {
      ...
      return json({ status: "degraded", health: "degraded", advisory: error.message, logs: [], checkedAt: (/* @__PURE__ */ new Date()).toISOString() });
    }
  }
  if (method === "GET" && pathname === "/api/os/cloudflare/metrics") {
    try {
      ...
      return json({ status: "degraded", health: "degraded", advisory: error.message, metrics: [], checkedAt: (/* @__PURE__ */ new Date()).toISOString() });
    }
  }
  ...
  if (method === "GET" && pathname === "/api/marketplace/service-modules") {
    return json({
      modules: [...serviceMarketplaceModules, auditLiteMarketplaceModule, promptInjectionMarketplaceModule]
    });
  }
