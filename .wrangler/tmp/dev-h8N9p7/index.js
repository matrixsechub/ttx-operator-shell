...
var AUDIT_LIFECYCLE_SEQUENCE = ["received", "validated", "queued", "processed"];
var AUDIT_LIFECYCLE_LABELS = {
  received: "Pending Intake",
  validated: "Validated",
  queued: "Queued",
  processed: "Processed"
};
var AUDIT_QUEUE_WEIGHT = {
  queued: 0,
  validated: 1,
  received: 2,
  processed: 3
...
__name(cloneTopRisks, "cloneTopRisks");
...
function ensureAuditLiteSubmission(details = {}) {
  ...
  return upsertAuditLiteSubmission({
    audit_id: auditId,
    created_at: details.created_at || (/* @__PURE__ */ new Date()).toISOString(),
    source_route: normalizeText2(details.source_route || details.source) || "audit-lite",
    company_type: normalizeText2(details.company_type) || "",
    ai_tools_used: normalizeArray2(details.ai_tools_used),
    data_used_with_ai: normalizeArray2(details.data_used_with_ai),
    ai_exposure: normalizeText2(details.ai_exposure),
    governance_controls: normalizeArray2(details.governance_controls),
    main_concern: normalizeText2(details.main_concern),
    ...
    recommended_service: normalizeText2(details.recommended_service) || "ai_security_audit",
    next_route: normalizeText2(details.next_route),
    engagement_id: normalizeNullable2(details.engagement_id || details.engagementId),
    status: "audit-lite-complete",
...
__name(ensureAuditLiteSubmission, "ensureAuditLiteSubmission");
...
__name(incrementHealthCounter, "incrementHealthCounter");
function advanceAuditLiteLifecycle(auditId, targetStatus, patch = {}) {
  const normalizedId = normalizeText2(auditId);
  ...
  const now = patch.updated_at || (/* @__PURE__ */ new Date()).toISOString();
  ...
  submission.status = normalizeText2(patch.status) || submission.status || "audit-lite-complete";
  submission.engagement_id = normalizeNullable2(patch.engagement_id || patch.engagementId) || submission.engagement_id || null;
  submission.risk_score = Number.isFinite(Number(patch.risk_score)) ? Number(patch.risk_score) : submission.risk_score;
  submission.risk_tier = normalizeText2(patch.risk_tier) || submission.risk_tier || "low";
  submission.priority = normalizeText2(patch.priority) || submission.priority || "low";
  submission.recommended_service = normalizeText2(patch.recommended_service || patch.recommendedService) || submission.recommended_service || "ai_security_audit";
...
__name(advanceAuditLiteLifecycle, "advanceAuditLiteLifecycle");
...
function attachEngagementToAuditLite(details = {}) {
  ...
  return advanceAuditLiteLifecycle(auditId, "queued", {
    ...
    status: normalizeText2(details.status) || "intake-received",
...
__name(markAuditLiteProcessed, "markAuditLiteProcessed");
...
__name(getAuditLiteSubmission, "getAuditLiteSubmission");
function buildLifecycleSnapshot(submission = {}, engagement = null) {
  const lifecycleStatus = normalizeText2(submission.lifecycle_status) || "received";
  return {
...
__name(buildLifecycleSnapshot, "buildLifecycleSnapshot");
function getAuditLiteLifecycle(auditId, engagements3 = []) {
  const submission = getAuditLiteSubmission(auditId);
  if (!submission) {
    return null;
  }
  const engagement = engagements3.find((entry) => entry.auditId === submission.audit_id) || null;
  return buildLifecycleSnapshot(submission, engagement);
}
__name(getAuditLiteLifecycle, "getAuditLiteLifecycle");
function listAuditLiteQueue(engagements3 = [], options = {}) {
  const shouldMarkProcessed = Boolean(options.markProcessed);
  const queue = [];
  for (const submission of auditLiteSubmissions) {
    const linkedEngagement = engagements3.find((entry) => entry.auditId && entry.auditId === submission.audit_id) || null;
    const activeSubmission = shouldMarkProcessed && linkedEngagement && submission.lifecycle_status === "queued" ? markAuditLiteProcessed(submission.audit_id) : submission;
    queue.push({
  ...
  for (const engagement of engagements3) {
    ...
    const submission = attachEngagementToAuditLite({
      ...
      created_at: engagement.createdAt || (/* @__PURE__ */ new Date()).toISOString()
    });
    queue.push({
      audit_id: engagement.auditId,
      engagement_id: engagement.id,
...
function buildRiskTierDistribution(rows = []) {
  return rows.reduce(
    (distribution, row) => {
      const tier = normalizeText2(row.risk_tier) || "low";
...
__name(buildRiskTierDistribution, "buildRiskTierDistribution");
function buildLifecycleDistribution(rows = []) {
  return rows.reduce(
    (distribution, row) => {
      const status = normalizeText2(row.lifecycle_status) || "received";
...
__name(buildLifecycleDistribution, "buildLifecycleDistribution");
function getAuditLiteHealthRecord() {
  return { ...auditLiteHealth };
}
__name(getAuditLiteHealthRecord, "getAuditLiteHealthRecord");
function getAuditLiteOperatorSnapshot(engagements3 = [], options = {}) {
  const rows = listAuditLiteQueue(engagements3, options);
  return {
    rows,
    summary: {
      total: rows.length,
...
__name(getAuditLiteOperatorSnapshot, "getAuditLiteOperatorSnapshot");
function getAuditLiteMarketplaceSummary(engagements3 = []) {
  const rows = listAuditLiteQueue(engagements3);
...
__name(getAuditLiteMarketplaceSummary, "getAuditLiteMarketplaceSummary");
...
// worker/data/intakeDemoMode.js
function isIntakeDemoMode(env) {
  return String(env?.INTAKE_AGENT_DEMO_MODE || "").toLowerCase() === "true";
}
__name(isIntakeDemoMode, "isIntakeDemoMode");
var INTAKE_DEMO_MODE_MESSAGE = "Demo Mode \u2014 No data stored";
var COCKPIT_STATUS = "under_construction";
var COCKPIT_MESSAGE = "Operator Cockpit is coming online soon \u2014 registration ensures you receive updates.";
...
__name(getPublicDemoModePayload, "getPublicDemoModePayload");

// worker/data/publicRegister.js
var SecurityLifecycleStage = {
  INTAKE: "intake",
  VALIDATION: "validation",
  QUEUED: "queued",
  PROCESSED: "processed"
};
var REGISTER_LIFECYCLE_SEQUENCE = ["received", "validated", "queued", "processed"];
var REGISTER_LIFECYCLE_LABELS = {
  received: "Pending Intake",
  validated: "Validated",
  queued: "Queued",
  processed: "Processed"
};
var REGISTER_QUEUE_WEIGHT = {
  queued: 0,
  validated: 1,
  received: 2,
  processed: 3
};
var allowedRoles = /* @__PURE__ */ new Set(["operator", "builder", "partner", "observer"]);
var AgentPermissionValidator = class {
  static {
    __name(this, "AgentPermissionValidator");
  }
...
var publicRegisterMarketplaceModule = {
...
__name(generatePublicRegisterId, "generatePublicRegisterId");
function ensureTimeline2(submission = {}) {
  if (!Array.isArray(submission.lifecycle_timeline)) {
    submission.lifecycle_timeline = [];
  }
  return submission.lifecycle_timeline;
}
__name(ensureTimeline2, "ensureTimeline");
function upsertPublicRegisterSubmission(patch = {}) {
  const registerId = normalizeText12(patch.register_id);
  if (!registerId) {
  ...
  const submission = {
    ...
    created_at: patch.created_at || existing?.created_at || (/* @__PURE__ */ new Date()).toISOString(),
    name: normalizeText12(patch.name) || existing?.name || "",
    email: normalizeText12(patch.email) || existing?.email || "",
    role: normalizeText12(patch.role) || existing?.role || "",
    reason: normalizeText12(patch.reason) || existing?.reason || "",
    source: normalizeText12(patch.source) || existing?.source || "public-register",
    priority: normalizeText12(patch.priority) || existing?.priority || "standard",
    recommended_service: normalizeText12(patch.recommended_service) || existing?.recommended_service || "ecosystem_access",
    next_route: normalizeText12(patch.next_route) || existing?.next_route || null,
    engagement_id: patch.engagement_id || existing?.engagement_id || null,
    status: normalizeText12(patch.status) || existing?.status || "register-pending",
    lifecycle_status: normalizeText12(patch.lifecycle_status) || existing?.lifecycle_status || "received",
    lifecycle_label: patch.lifecycle_label || existing?.lifecycle_label || REGISTER_LIFECYCLE_LABELS.received,
    lifecycle_timeline: Array.isArray(patch.lifecycle_timeline) ? patch.lifecycle_timeline.map((entry) => ({ ...entry })) : ensureTimeline2(existing || {}).map((entry) => ({ ...entry }))
  };
  if (existingIndex >= 0) {
...
__name(upsertPublicRegisterSubmission, "upsertPublicRegisterSubmission");
function advancePublicRegisterLifecycle(registerId, targetStatus, patch = {}) {
  const normalizedId = normalizeText12(registerId);
  ...
  const timeline = ensureTimeline2(submission);
  for (let index = 0; index <= targetIndex; index += 1) {
    ...
    timeline.push({
      ...
      at: patch.created_at || (/* @__PURE__ */ new Date()).toISOString(),
  ...
  submission.status = normalizeText12(patch.status) || submission.status || "register-pending";
...
__name(advancePublicRegisterLifecycle, "advancePublicRegisterLifecycle");
function normalizePublicRegisterAnswers(payload = {}) {
  const name = normalizeText12(payload.name);
  const email = normalizeText12(payload.email);
  const role = normalizeText12(payload.role).toLowerCase();
  const reason = normalizeText12(payload.reason);
  if (!name) {
  ...
  return {
    ...
    role,
    reason,
    source: normalizeText12(payload.source) || "public-register",
    source_route: normalizeText12(payload.source_route) || "/register"
  };
}
__name(normalizePublicRegisterAnswers, "normalizePublicRegisterAnswers");
...
__name(derivePriorityFromRole, "derivePriorityFromRole");
function computePublicRegisterResult(answers, registerId = generatePublicRegisterId()) {
...
__name(computePublicRegisterResult, "computePublicRegisterResult");
function recordPublicRegisterSubmission(answers, result) {
  const submission = upsertPublicRegisterSubmission({
...
__name(recordPublicRegisterSubmission, "recordPublicRegisterSubmission");
function getPublicRegisterSubmission(registerId) {
  const normalizedId = normalizeText12(registerId);
  if (!normalizedId) {
    return null;
  }
...
__name(getPublicRegisterSubmission, "getPublicRegisterSubmission");
function buildLifecycleSnapshot2(submission = {}) {
  const lifecycleStatus = normalizeText12(submission.lifecycle_status) || "received";
  return {
    ...
    lifecycle_timeline: ensureTimeline2(submission).map((entry) => ({ ...entry })),
    observer_mode: lifecycleStatus === "received" || lifecycleStatus === "validated",
    operator_mode: lifecycleStatus === "processed",
    permission_profile: publicRegisterMarketplaceModule.permission_profile,
    security_stage: publicRegisterMarketplaceModule.security_stage,
...
__name(buildLifecycleSnapshot2, "buildLifecycleSnapshot");
function getPublicRegisterLifecycle(registerId) {
  const submission = getPublicRegisterSubmission(registerId);
  if (!submission) {
    return null;
  }
  return buildLifecycleSnapshot2(submission);
}
__name(getPublicRegisterLifecycle, "getPublicRegisterLifecycle");
function getPublicRegisterSecurityPlane() {
  return {
    module: {
...
__name(getPublicRegisterSecurityPlane, "getPublicRegisterSecurityPlane");
function listPublicRegisterQueue() {
  return [...publicRegisterSubmissions].map((submission) => ({
    register_id: submission.register_id,
...
__name(listPublicRegisterQueue, "listPublicRegisterQueue");
function getPublicRegisterQueuePreview() {
...
__name(getPublicRegisterQueuePreview, "getPublicRegisterQueuePreview");
function buildLifecycleDistribution2(rows = []) {
  return rows.reduce(
    (distribution, row) => {
      const status = normalizeText12(row.lifecycle_status) || "received";
      distribution[status] = (distribution[status] || 0) + 1;
      return distribution;
    },
    { received: 0, validated: 0, queued: 0, processed: 0 }
...
__name(buildLifecycleDistribution2, "buildLifecycleDistribution");
function getPublicRegisterOperatorSnapshot(options = {}) {
  const rows = listPublicRegisterQueue();
  const shouldMarkProcessed = Boolean(options.markProcessed);
  const visibleRows = shouldMarkProcessed ? rows.map((row) => {
  ...
  return {
    ...
    summary: {
      ...
      lifecycle_distribution: buildLifecycleDistribution2(visibleRows),
      security_plane: getPublicRegisterSecurityPlane(),
      read_only: true
    }
  };
}
__name(getPublicRegisterOperatorSnapshot, "getPublicRegisterOperatorSnapshot");
function getPublicRegisterMarketplaceSummary() {
  const rows = listPublicRegisterQueue();
  const latest = rows[0] || null;
  return {
    total_submissions: rows.length,
...
__name(getPublicRegisterMarketplaceSummary, "getPublicRegisterMarketplaceSummary");
function buildPublicRegisterMarketplacePayload() {
  return {
...
__name(buildPublicRegisterMarketplacePayload, "buildPublicRegisterMarketplacePayload");
var publicRegister_default = {
...
var {
  ...
  getAuditLiteLifecycle: getAuditLiteLifecycle2,
  getAuditLiteOperatorSnapshot: getAuditLiteOperatorSnapshot2,
  getAuditLiteMarketplaceSummary: getAuditLiteMarketplaceSummary2
} = auditLite_default;
var {
  publicRegisterMarketplaceModule: publicRegisterMarketplaceModule2,
  normalizePublicRegisterAnswers: normalizePublicRegisterAnswers2,
  computePublicRegisterResult: computePublicRegisterResult2,
  recordPublicRegisterSubmission: recordPublicRegisterSubmission2,
  getPublicRegisterLifecycle: getPublicRegisterLifecycle2,
  getPublicRegisterSecurityPlane: getPublicRegisterSecurityPlane2,
  getPublicRegisterQueuePreview: getPublicRegisterQueuePreview2,
  getPublicRegisterOperatorSnapshot: getPublicRegisterOperatorSnapshot2,
  getPublicRegisterMarketplaceSummary: getPublicRegisterMarketplaceSummary2,
  buildPublicRegisterMarketplacePayload: buildPublicRegisterMarketplacePayload2
} = publicRegister_default;
var {
...
function normalizeNullable3(value) {
  const normalized = normalizeText13(value);
  return normalized || null;
}
__name(normalizeNullable3, "normalizeNullable");
function normalizeEmail(value) {
  return normalizeText13(value).toLowerCase();
}
__name(normalizeEmail, "normalizeEmail");
function normalizeAnnotationPayload(payload) {
  return {
    annotation: normalizeText13(payload.annotation || payload.note)
  };
}
...
function normalizeIdentityPayload(payload, fallbackSourcePage, request) {
  const identity = {
    ...
    operator_handle: normalizeText13(payload.operator_handle || payload.name || payload.operatorHandle),
    organization: normalizeNullable3(payload.organization || payload.org || payload.company),
    contact_email: normalizeEmail(payload.contact_email || payload.email || payload.contactEmail),
    transmission: normalizeText13(payload.transmission || payload.message || payload.notes),
    source_page: normalizeText13(payload.source_page || payload.source || fallbackSourcePage) || void 0,
    package_interest: normalizeNullable3(payload.package_interest || payload.packageId || payload.packageIdValue),
    module_interest: normalizeNullable3(payload.module_interest || payload.moduleId),
    urgency: normalizeNullable3(payload.urgency),
    utm_source: normalizeNullable3(payload.utm_source),
    utm_medium: normalizeNullable3(payload.utm_medium),
    auto_reply_sent: Boolean(payload.auto_reply_sent),
    contacted_at: payload.contacted_at || (/* @__PURE__ */ new Date()).toISOString(),
    ip_address: request.headers.get("CF-Connecting-IP"),
    status: normalizeText13(payload.status) || "new"
  };
...
function normalizeEngagementPayload(payload) {
  return {
    packageId: normalizeNullable3(payload.package_interest || payload.packageId),
    operatorHandle: normalizeText13(payload.operator_handle || payload.name || payload.operatorHandle || payload.contactName),
    organization: normalizeNullable3(payload.organization || payload.org || payload.company),
    contactEmail: normalizeEmail(payload.contact_email || payload.email || payload.contactEmail),
    transmission: normalizeText13(payload.transmission || payload.message || payload.notes),
    source: normalizeText13(payload.source || payload.source_page) || "landing",
    moduleInterest: normalizeNullable3(payload.module_interest || payload.moduleId),
    urgency: normalizeNullable3(payload.urgency),
    selectorId: normalizeNullable3(payload.selector_id || payload.selectorId),
    recommendedService: normalizeNullable3(payload.recommended_service || payload.recommendedService || payload.service),
    secondaryService: normalizeNullable3(payload.secondary_service || payload.secondaryService),
    priority: normalizeNullable3(payload.priority),
    revenuePotential: normalizeNullable3(payload.revenue_potential || payload.revenuePotential),
    urgencyScore: Number.isFinite(Number(payload.urgency_score)) ? Number(payload.urgency_score) : null,
    auditId: normalizeNullable3(payload.audit_id || payload.auditId),
    scanId: normalizeNullable3(payload.scan_id || payload.scanId),
    agentCheckId: normalizeNullable3(payload.agent_check_id || payload.agentCheckId),
    automationRoiId: normalizeNullable3(payload.automation_roi_id || payload.automationRoiId),
    ragRiskId: normalizeNullable3(payload.rag_risk_id || payload.ragRiskId),
    ...
    riskTier: normalizeNullable3(payload.risk_tier || payload.riskTier),
    readinessTier: normalizeNullable3(payload.readiness_tier || payload.readinessTier),
    roiTier: normalizeNullable3(payload.roi_tier || payload.roiTier),
    ragRiskTier: normalizeNullable3(payload.rag_risk_tier || payload.ragRiskTier),
    ...
    retrievalExposureLevel: normalizeNullable3(payload.retrieval_exposure_level || payload.retrievalExposureLevel),
    accessControlLevel: normalizeNullable3(payload.access_control_level || payload.accessControlLevel),
    governanceMaturity: normalizeNullable3(payload.governance_maturity || payload.governanceMaturity),
    buildComplexity: normalizeNullable3(payload.build_complexity || payload.buildComplexity),
    automationComplexity: normalizeNullable3(payload.automation_complexity || payload.automationComplexity),
    safetyLevel: normalizeNullable3(payload.safety_level || payload.safetyLevel)
  };
}
__name(normalizeEngagementPayload, "normalizeEngagementPayload");
function normalizeIntentPayload(payload) {
  return {
    intent: normalizeText13(payload.intent),
    source: normalizeText13(payload.source) || "public-landing",
    agent: normalizeText13(payload.agent) || "route-advisory"
  };
}
...
async function createIntegration(env, payload) {
  const endpoint = normalizeText13(payload.endpoint);
  const integration = {
    id: buildId("int"),
    name: normalizeText13(payload.name) || "Unnamed Integration",
    type: normalizeText13(payload.type) || "api",
    endpoint,
    permissions: Array.isArray(payload.permissions) ? payload.permissions.map((entry) => normalizeText13(entry)).filter(Boolean) : [],
    createdAt: (/* @__PURE__ */ new Date()).toISOString()
  };
...
async function resolveIntegrationContext(env, value, requiredPermission) {
  const integrationId = normalizeText13(value);
  if (!integrationId) {
    return null;
  }
...
async function evaluateSafetyCheck(env, payload) {
  ...
  const intent = normalizeText13(payload.intent);
  const agent = normalizeText13(payload.agent);
  const pipelineName = normalizeText13(payload.pipeline);
  const scenarioName = normalizeText13(payload.scenario);
  const reasons = [];
  if (agent) {
    const rule = governance.agentSafetyRules?.[agent];
    if (rule?.enabled === false) {
...
async function enforceGovernance(env, surface, payload, options = {}) {
  ...
  const agent = normalizeText13(options.agent || payload.agent);
  if (integrationId) {
...
async function runCertification(env, payload) {
  ...
  const tests = Array.isArray(payload.tests) && payload.tests.length ? payload.tests.map((entry) => normalizeText13(entry)).filter(Boolean) : ["route-smoke", "payload-shape", "safety-compat"];
  const results = [];
...
async function applyRelease(env, payload) {
  const version = normalizeText13(payload.version);
  if (!version) {
    throw new Error("version is required");
  }
  const changes = Array.isArray(payload.changes) ? payload.changes.map((entry) => normalizeText13(entry)).filter(Boolean) : [];
  const migrations = Array.isArray(payload.migrations) ? payload.migrations.map((entry) => normalizeText13(entry)).filter(Boolean) : [];
  const release = {
    id: `rel-${Date.now()}`,
    version,
    changes,
...
async function computeGlobalRoutePlan(env, payload) {
  ...
  const normalizedIntent = normalizeText13(payload.intent);
  const mode = normalizeText13(payload.mode) || "auto";
...
async function saveDivisionMemoryEntry(env, payload) {
  const key = normalizeText13(payload.key || payload.id);
  if (!key) {
  ...
  const record = {
    id: key,
    key,
    type: normalizeText13(payload.type) || "operator-note",
    value: payload.value ?? payload.note ?? payload.metadata ?? null,
    summary: normalizeText13(payload.summary) || normalizeText13(typeof payload.value === "string" ? payload.value : key),
    updatedAt: (/* @__PURE__ */ new Date()).toISOString()
  };
...
async function runPipeline(env, payload, source = "pipeline-engine") {
  const moduleId = normalizeText13(payload.moduleId);
  ...
  const governance = await enforceGovernance(
    ...
    {
      ...payload,
      intent: normalizeText13(payload.intent) || `Pipeline for ${moduleId}`,
      integrationId: getIntegrationIdFromPayload(payload)
    },
    {
      requestedChainLength: steps.length,
      requiredPermission: "pipeline",
      chain: steps.map((step) => normalizeText13(step.agent)).filter(Boolean)
    }
  );
  await assertCertifiedAgents(
    env,
    steps.map((step) => normalizeText13(step.agent)).filter(Boolean),
    "pipeline",
    governance
  );
  ...
  for (const step of steps) {
    ...
    const flow = await executeAgentFlow(
      env,
      {
        intent: normalizeText13(step.intent) || moduleRecord?.summary || `Pipeline for ${moduleId}`,
        source,
        agent: step.agent
      },
      {
...
async function runSandbox(env, payload) {
  const agent = normalizeText13(payload.agent);
  const input = normalizeText13(payload.input);
...
async function createOperatorIntent(env, payload) {
  const goal = normalizeText13(payload.goal);
  if (!goal) {
  ...
  const routePlan = await computeGlobalRoutePlan(env, {
    ...
    mode: "manual",
    context: {
      priority: normalizeText13(payload.priority) || "medium",
...
async function runPublicScenario(env, payload) {
  const name = normalizeText13(payload.name);
  const steps = Array.isArray(payload.steps) ? payload.steps : [];
  ...
  const governance = await enforceGovernance(
    ...
    },
    {
      ...
      chain: steps.map((step) => lightweightSemanticScore(normalizeText13(step.intent)).recommendedAgent)
    }
  );
  const plannedAgents = [];
  for (const step of steps) {
    const previewPlan = await computeGlobalRoutePlan(env, {
      intent: normalizeText13(step.intent),
      mode: "auto",
...
async function runScenario(env, payload) {
  ...
  for (const step of steps) {
    ...
    const intent = normalizeText13(step.intent) || normalizeText13(previousOutput?.envelope?.content) || name;
    const flow = await executeAgentFlow(
      env,
...
async function executeOperatorCommand(env, commandText) {
  const command = normalizeText13(commandText);
  if (!command) {
...
async function handleMarketplaceSearch(request, env, url, logSearches = true) {
  const term = normalizeText13(url.searchParams.get("q"));
...
async function handleApi(request, env, url) {
  ...
  if (method === "POST" && pathname === "/api/audit-lite") {
    try {
      ...
      const lifecycle = getAuditLiteLifecycle2(result.audit_id, engagements2);
      return json2(
        {
          ...result,
          lifecycle,
          hq_health: getAuditLiteOperatorSnapshot2(engagements2).summary.health,
          route_table: intakeAgent_default.intakeRoutingTable?.audit_lite || null
        },
        200,
        { "Cache-Control": "no-store" }
      );
  ...
  if (method === "GET" && pathname === "/api/audit-lite/lifecycle") {
    const auditId = normalizeText13(url.searchParams.get("audit_id"));
    if (!auditId) {
      return json2({ error: "audit_id is required" }, 400);
    }
    const lifecycle = getAuditLiteLifecycle2(auditId, engagements2);
    if (!lifecycle) {
      return json2({ error: "Audit lifecycle not found" }, 404);
    }
    return json2(lifecycle, 200, { "Cache-Control": "no-store" });
  }
  ...
  if (method === "POST" && pathname === "/api/os/version") {
    try {
      ...
      const nextVersion = {
        current: normalizeText13(body.current) || current.current,
        history: Array.isArray(body.history) && body.history.length ? body.history : [.../* @__PURE__ */ new Set([...current.history, normalizeText13(body.current) || current.current])],
  ...
  if (method === "GET" && pathname === "/api/events") {
    try {
      const type = normalizeText13(url.searchParams.get("type"));
  ...
  if (method === "GET" && pathname === "/api/operator/audit") {
    try {
      const action = normalizeText13(url.searchParams.get("action"));
  ...
  if (method === "GET" && pathname === "/api/public/demo-mode") {
    return json2(getPublicDemoModePayload(env), 200, { "Cache-Control": "no-store" });
  }
  if (method === "POST" && pathname === "/api/register") {
    try {
      const payload = await readBody2(request);
      const demoMode = isIntakeDemoMode(env);
      const answers = normalizePublicRegisterAnswers2(payload);
      const result = computePublicRegisterResult2(answers);
      let lifecycle = null;
      if (!demoMode) {
        recordPublicRegisterSubmission2(answers, result);
        lifecycle = getPublicRegisterLifecycle2(result.register_id);
      } else {
        lifecycle = {
          register_id: result.register_id,
          lifecycle_status: "received",
      ...
      return json2(
        {
          ...
          ...result,
          lifecycle,
          security_plane: getPublicRegisterSecurityPlane2(),
          queue_preview: demoMode ? { queue_length: 0, lifecycle_stage: "received" } : getPublicRegisterQueuePreview2(),
          route_table: intakeAgent_default.intakeRoutingTable?.public_register || null
        },
        200,
        { "Cache-Control": "no-store" }
      );
    } catch (error) {
      return json2({ error: error.message || "register-failed" }, 400);
    }
  }
  if (method === "GET" && pathname === "/api/register-lifecycle") {
    const registerId = normalizeText13(url.searchParams.get("register_id"));
    if (!registerId) {
      return json2({ error: "register_id is required" }, 400);
    }
    const lifecycle = getPublicRegisterLifecycle2(registerId);
    if (!lifecycle) {
      return json2({ error: "Register lifecycle not found" }, 404);
    }
    return json2(lifecycle, 200, { "Cache-Control": "no-store" });
  }
  if (method === "GET" && pathname === "/api/register-security") {
    return json2(getPublicRegisterSecurityPlane2(), 200, { "Cache-Control": "no-store" });
  }
  if (method === "GET" && pathname === "/api/register-queue") {
    return json2(getPublicRegisterQueuePreview2(), 200, { "Cache-Control": "no-store" });
  }
  ...
  if (method === "PATCH" && pathname === "/api/operator/service-intake/status") {
    try {
      const payload = await readBody2(request);
      const selectorId = normalizeText13(payload.selector_id || payload.selectorId);
      const status = normalizeText13(payload.status);
  ...
  if (method === "POST" && pathname === "/api/cloudflare/security-audit/start") {
    ...
    try {
      const payload = await readBody2(request);
      const engagementId = normalizeText13(payload.engagement_id || payload.engagementId);
      if (!engagementId) {
  ...
  if (method === "GET" && pathname === "/api/operator/audit-lite") {
    return json2(
      getAuditLiteOperatorSnapshot2(engagements2, {
        markProcessed: isOperatorSurfaceRequest2(request)
      }),
      200,
      { "Cache-Control": "no-store" }
    );
  ...
  if (method === "GET" && pathname === "/api/operator/register-intake") {
    return json2(
      getPublicRegisterOperatorSnapshot2({
        markProcessed: isOperatorSurfaceRequest2(request)
      }),
      200,
      { "Cache-Control": "no-store" }
    ...
    return json2({
      modules: [
        ...serviceMarketplaceModules2,
        {
          ...
          lifecycle_summary: getAuditLiteMarketplaceSummary2(engagements2)
        },
        ...
        buildPublicRegisterMarketplacePayload2()
      ]
    });
  }
...
// ../../AppData/Local/npm-cache/_npx/81c4f9ac33f52c68/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../AppData/Local/npm-cache/_npx/81c4f9ac33f52c68/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-YapYfk/middleware-insertion-facade.js
...
// ../../AppData/Local/npm-cache/_npx/81c4f9ac33f52c68/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-YapYfk/middleware-loader.entry.ts
